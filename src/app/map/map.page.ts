...
export class MapPage implements OnInit {
  ...
  addMarker(marker: Marker) {
    var localizador = new google.maps.Marker({
      position: marker.position,
      map: this.map,
      title: marker.title
    });
    this.addInfoToMarker(marker, localizador);
    return localizador;
  }

  addInfoToMarker(marker: Marker, localizador: any) {
    this.mapController.getHttpData(marker).subscribe((coordData: any) => {
      ...
      localizador.addListener('click', () => {
        infoWindow.open(this.map, localizador);
      });
